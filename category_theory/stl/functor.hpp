...
template <
    class F,
    class O,
    class T = category_theory::functor_value_t<O>,
...
R category_theory_functor_custom_map(F&& f, const V& v) {
  ...
  us.reserve(v.size());
  std::transform(
      std::begin(v), std::end(v), std::back_inserter(us), std::forward<F>(f));
  return us;
}
...
std::vector<U, Ps...> category_theory_functor_custom_map(
    F&& f, std::vector<U, Ps...>&& v) {
  // Garantees no self move, which would be UB
  std::vector<U, Ps...> n = std::move(v);
  for (auto& e : n) e = f(e);
  return n;
}
