...
using make_functor_t = typename functor_traits<
    std::remove_reference_t<F>>::template make_functor<T>;

template <class F>
using functor_value_t =
    typename functor_traits<std::remove_reference_t<F>>::value;

template <typename T, template <typename...> class F>
struct is_of_functor_type : std::is_same<apply_template_parameters<T, F>, T> {};

template <typename T, template <typename...> class F>
constexpr static inline bool is_of_functor_type_v =
    is_of_functor_type<T, F>::value;
