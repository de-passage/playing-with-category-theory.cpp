...
template <class, template <typename...> class> struct apply_template_parameters;

template <template <typename...> class T, template <class> class F,
          typename... Ps>
...
template <class T> using never_t = std::enable_if_t<!std::is_same_v<T, T>>;

template <class T> constexpr static bool fail = false;
...
struct is_direct_list_initializable<
    T, U<Ts...>, std::void_t<decltype(T{std::declval<Ts>()...})>>
    : std::true_type {};
...
struct is_direct_list_initializable : detail::is_direct_list_initializable<T, tuple<Ts...>>  {};

template <class T, class... Ts>
constexpr static inline bool is_direct_list_initializable_v = is_direct_list_initializable<T, Ts...>::value;
