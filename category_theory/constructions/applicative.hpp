...
template <template <typename> class A> struct pure_t {
  template <class T, class = std::enable_if_t<
                         fits_auto_implementation_requirements<A, T>>>
  ...
  [[nodiscard]] constexpr A<T> operator()(T &&value) const noexcept {
    static_assert(fail<T>,
                  "Applicative types need to either be either "
                  "constructible from a type matching their template "
                  "parameter, or have a "
